<template>
  <SettingsTabLoader :fetcher="fetchData" empty-text="暂无物品记录">
    <template #default="{ data }">
      <EntityTable :account="'items'" :entries="entriesOf(data)" />
    </template>
  </SettingsTabLoader>
</template>
...
const fetchData = async () => (await getDynamicAccount(projectId, 'items')).data.data
